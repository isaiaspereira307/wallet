use crate::filemanager::filemanager::ler_arquivo;
use crate::objects::objects::{Transacao, Operacao, Tipo, Investimento, TipoInvestimento};
use chrono::{Local, Datelike};


pub fn calcular_receita_despesa() -> (f32, f32) {
    let transactions = ler_arquivo::<Transacao>("trasaction.json".to_string());
    let month = Local::now().month();
    let receitas_fixas  = transactions.iter()
        .filter(|transacao| transacao.operacao == Operacao::Deposito && transacao.tipo == Tipo::ValorFixo)
        .fold(0.0, |acc, transacao| acc + transacao.valor);
    let receitas_variadas  = transactions.iter()
        .filter(|transacao| transacao.operacao == Operacao::Deposito && transacao.tipo == Tipo::ValorVariavel && transacao.mes.unwrap() == month)
        .fold(0.0, |acc, transacao| acc + transacao.valor);
    let despesas_fixas = transactions.iter()
        .filter(|transacao| transacao.operacao == Operacao::Resgate && transacao.tipo == Tipo::ValorFixo)
        .fold(0.0, |acc, transacao| acc + transacao.valor);
    let despesas_variadas = transactions.iter()
        .filter(|transacao| transacao.operacao == Operacao::Resgate && transacao.tipo == Tipo::ValorVariavel && transacao.mes.unwrap() == month)
        .fold(0.0, |acc, transacao| acc + transacao.valor);
